...
<div on:mouseup={() => handleMouseUp()} on:keydown={handleKeydown} role="button" tabindex="0" class="cursor-default flex flex-col justify-between items-center w-screen h-screen m-auto transition-all duration-75 {$theme ? 'bg-neutral-900' : 'bg-white'}">
    ...
    <div class="absolute {!info ? 'pointer-events-none' : ''} z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-72">
        <div class="{info ? 'opacity-100' : 'opacity-0 translate-y-4'} transition-all duration-200 flex flex-col justify-center items-center text-center { $theme == 0 ? 'bg-white border-black text-black' : 'bg-neutral-900 border-white text-white' } p-4 rounded-lg border-2">
            <p class="text-lg mb-2">
                To win the game you need to place a queen on each row, column and color.
            </p>
            
            <button on:click={toggleInfo} class="transition-colors duration-200 border-2 rounded-lg px-4 py-2 mb-2 mt-4 text-black
                { $theme == 0 ? 
    ...
    <!-- {#if info || win || closeWindowMessage}
        <div class="absolute w-screen h-screen m-auto transition-all duration-75 bg-black/50">
        </div>
    {/if} -->

    <div class="absolute {!closeWindowMessage ? 'pointer-events-none' : ''} z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-72">
    ...
    {#if board}
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center items-center gap-4 m-auto">
            ...
            <div class="flex flex-row m-auto border-4 border-white">
                {#each board.cells as row, i}
                    <div class="flex flex-col">
                        {#each row as cell, j}
                            <!-- {@const leftBorder = i === 0 || board.cells[i - 1][j].color != board.cells[i][j].color ? 
                                $theme == 0 ? 'border-l-4 border-l-black' : 'border-l-2 border-l-white' :
                                $theme == 0 ? 'border-l border-black' : 'border-l border-white'
                            }
                            {@const rightBorder = i === board.cells.length - 1 ? 
                                $theme == 0 ? 'border-r-4 border-r-black' : 'border-r-2 border-r-white' :
                                ''
                            }
                            {@const topBorder = j === 0 || board.cells[i][j - 1].color != board.cells[i][j].color ? 
                                $theme == 0 ? 'border-t-4 border-t-black' : 'border-t-2 border-t-white' :
                                $theme == 0 ? 'border-t border-black' : 'border-t border-white'
                            }
                            {@const bottomBorder = j === board.cells[0].length - 1 ? 
                                $theme == 0 ? 'border-b-4 border-b-black' : 'border-b-2 border-b-white' :
                                ''
                            } -->
                            ...
                            <button 
                                class="{n == 5 || n == 6 ? 'size-16' : 'size-12'} {cell.color} border-0 {leftBorder} {rightBorder} {topBorder} {bottomBorder} m-auto"
                                on:mousedown|preventDefault={(event) => handleMouseDown(event, i, j)}
                                on:mouseenter={() => handleMouseEnter(i, j)}
                            >
                                {#if cell.state === 'queen'}
                                    <img class="{n == 5 || n == 6 ? 'size-16' : 'size-12'} p-1" src="/pieces/wQ.svg" alt="white queen"/>
